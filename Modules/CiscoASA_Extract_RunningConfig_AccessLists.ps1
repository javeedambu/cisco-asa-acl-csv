########################################################################
#
# Script Name: CiscoASA_Extract_RunningConfig_AccessLists.ps1
# Description: Script to extract ACLs from the from SHOW TECSUPPORT.
#
# Instrunction:
...
$startMarker = "access-list "
$endMarker = "pager lines"
...
Get-Content -Path $filePath | ForEach-Object {
    ...
    if ($_.StartsWith("hostname ")) {
        $runningConfigFileName = "./Output/RunningConfig_$(($_ -Split " ")[1])_AccessLists_$(Get-Date -format yyyy-MM-dd-HHmmss).txt"
        $extractedLines += $_
    ...
    if ($_.StartsWith($endMarker)) { 
        ...
        if ($extractedLines) {
            $extractedLines | Out-File -FilePath $runningConfigFileName -Append
            Write-host "AccessLists File exported to $runningConfigFileName.`n"
            $extractedLines = @()
        }
        # End the loop
        return
